...
class History:
    ...
    def add(self, VL, left_lane, right_lane):
        ...
        if self._cntr < self.__history_length:
            ...
            self.VL = self.VL_hist[:,:,-self._cntr:].dot(self.__weight[-self._cntr:]) / W
            self.left_lane = self.left_lane_hist[:,:,-self._cntr:].dot(self.__weight[-self._cntr:]) / W
            self.right_lane =self.right_lane_hist[:,:,-self._cntr:].dot(self.__weight[-self._cntr:]) / W
        else:
...
def pre_process_image(I, roi, resize_factor = 1):
    ...
    if resize_factor != 1:
        ...
        roi = cv2.resize(roi.astype(np.float32),(0,0),fx = resize_factor,fy = resize_factor)
        roi = roi > 0
...
def compute_lines(vote_landscape, roi, previous_VL=None):
    ...
    wr = lstsq( X , y[inrange]+1 )[0] # This returns the intercept and slope of the line
    ...
    inrange = x < (x_mid - 10)
    X = np.column_stack( (np.ones(np.count_nonzero(inrange)), x[inrange]+1))
    wl = lstsq( X , y[inrange]+1 )[0] # This returns the intercept and slope of the line
...
def create_ROI(img_size, vertices):
    ...
    vrts = np.around(vertices*(img_size[1]-1,img_size[0]-1)).astype(np.int32)
...
def find_frames_lane_lines(I,roi,history):
    ...
    WORKING_IMAGE_WIDTH = 400 # The imageas do need to be so large, so scale them down to this size if larger
    ...
    history.mask = mask # saving the mask only because it will be helpful to plot while describing the method
